...
```{r}
fit.fixed<-summary(lm(logwage~numChildren +factor(PUBID) +factor(year),data=Panel.data))
fit.fixed$coefficients[1,c(1:4)]

```
## The coefficient of the number of children for each woman in agiven year is 5.734 and it is the increase in the number of wages by 5.734 cent when the number of children increase by one.
...
## Answer 4
## The standard error is 0.1682

```{r}
fit.fixed$coefficients[1,1]/ fit.fixed$coefficients[1,2]
```
## This is the value of the test statistics of the null hypothesis that coefficient of the number of children is zero.
## At alpha level of 0.001, the null hypothesis can still be rejected.
## To reject the null hypothesis means that the coefficient of the number children is statistically significant or not equal to zero.
## There may be other predictors which when included in the model may affect the coefficients (confounding variables)
...
```{r}
fit.fixed.location<-summary(lm(logwage~numChildren +region+urban+factor(PUBID) +factor(year),data=Panel.data))
fit.fixed.location$coefficients[1,c(1:4)]

```
## The coefficient is statistically significant since the p value is less than 0.05, implying that we reject the null hypothesis that the coefficient is not significant.
## The amount of hourly wages increases by 5.7168 cent when the number of children increase by 1.

```{r}
fit.fixed.location$coefficients[2,c(1:4)]
```
##  The coefficient is statistically significant since the p value is less than 0.05, implying that we reject the null hypothesis that the coefficient is not significant.
## The amount of hourly wages decreases by 7.5812 cents when they move from one region to another.

```{r}
fit.fixed.location$coefficients[3,c(1:4)]

```
##  The coefficient is not statistically significant since the p value is less than 0.05, implying that we accept the null hypothesis that the coefficient is not significant.
## The amount of wages had decrease by 0.01 cents when the mothers move from the urban regions.
```{r}

fit.fixed.Maital<-summary(lm(logwage~numChildren +marstat +factor(PUBID) +factor(year),data=Panel.data))
fit.fixed.Maital$coefficients[1,c(1:4)]
```
## The coefficient is statistically significant since p value is less than 0.05

```{r}
fit.fixed.human<-summary(lm(logwage~educ+school+experience+tenure+tenure2 +factor(PUBID) +factor(year),data=Panel.data))
fit.fixed.human$coefficients[1,c(1:4)]
```
## The coefficient is statistically significant since p value is less than 0.05
```{r}

fit.fixed.job<-summary(lm(logwage~fullTime +firmSize+multipleLocations+unionized+industry +factor(PUBID) +factor(year),data=Panel.data))
fit.fixed.job$coefficients[1,c(1:4)]
...
```{r}
fit.fixed.interact<-summary(lm(logwage~numChildren*regularity+numChildren*hazardous,data=Panel.data))
fit.fixed.interact$coefficients

```
## Base on the p-values, all of the five coefficients are statistically significant with the exception of the interaction between numchildren and hazardou whose p-value is greater than 0.05. 