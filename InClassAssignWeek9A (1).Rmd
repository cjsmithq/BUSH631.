...
```{r}
#The forcing variable is fpm and the outcome variable is the population. 
# Because the treated and non treated villages have different cut off points or have variations. This brings about difference between the treated and untreated villages and thus demands that we analyze them seperately. 
# RD distinguishes the two cut off points and analyzes them separately.
```
...
```{r}
corpt <- read.csv("corruption.csv")
fit.bf<-lm(broad~fpm,data=corpt)
fit.bf
fit.bfp<-lm(broad~fpm+pop,data=corpt)
fit.bfp
fit.bfpc<-lm(broad~fpm+pop+pop_cat,data=corpt)
fit.bfpc
```

```{r}
fit.nf<-lm(narrow~fpm,data=corpt)
fit.nf
fit.nfp<-lm(narrow~fpm+pop,data=corpt)
fit.nfp
fit.nfpc<-lm(narrow~fpm+pop+pop_cat,data=corpt)
fit.nfpc
```
